...
function calculateMessageDelay(text) {
    ...
    // Base 380ms + 18ms per character + small jitter (0~100ms)
    const calculated = 380 + (len * 18) + Math.floor(Math.random() * 100);
    // Min 450ms, Max 1600ms (1.6초 상한)
    return Math.min(1600, Math.max(450, calculated));
}
